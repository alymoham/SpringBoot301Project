...
import com.eatza.order.feinclients.FienClients;
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for(OrderedItemsDto itemDto: itemsDtoList) {
			...
			try {
				...
				System.out.println(restaurantServiceItemUrl+itemDto.getItemId());
		...
		feedback.setCategory("Menu");
		feedback.setDescription("Good Options");
		
		fienClients.addFeedback(feedback);
		logger.debug("Saved order to db");
		kafkaSender.send("Order Has Been Sent");
		logger.debug("Message sent to restaurant through kafka queue");
		return savedOrder;
	}
